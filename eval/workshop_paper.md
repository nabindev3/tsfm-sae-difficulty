...
3. A **causal check that hardens the null.** A Mishra-style hook ablation of the top-5 features first showed a directional +0.043 hint on hard windows (5/5 same sign); a pre-registered power analysis flagged it as underpowered (49 % power at n = 56), and a matched-100-sample re-run reversed it to −0.049 (CI [−0.086, −0.015]) — the hint was Monte-Carlo noise. The features are correlationally *and* causally null.
...
Per-feature Δ(ablate − recon) is null at the population level (all individual CIs cross zero). Stratifying by test-window difficulty reveals a directional pattern: on hard windows (top tercile by natural CRPS, n = 56), **5/5 features show positive point estimates** (+0.028 to +0.055); on easy windows (n = 111) the same features show ~null to slightly negative estimates. The aggregate test (mean ΔCRPS averaged across the 5 features) on hard windows reaches +0.043, 95 % CI [−0.008, +0.095] — directionally consistent and ~3 % of mean natural CRPS, but underpowered for individual-feature significance at n = 56. The diff-in-diff (hard − easy) is +0.050, 95 % CI [−0.005, +0.104]. **This directional hint did not survive scrutiny.** A pre-registered power analysis (report §4.8) showed n = 56 gave only 49 % power; a matched-Monte-Carlo re-run with *every* condition at 100 samples (report §4.13) then reversed the hard-window aggregate to **−0.049, 95 % CI [−0.086, −0.015]** (diff-in-diff −0.070, z = −3.49), with the reconstruction baseline still null (+0.006). The 50-sample +0.043 was Monte-Carlo noise, not signal. Conclusion: the top-5 features carry **no robust causal contribution** — the §4.2 correlational null is *not* rescued by causation, a cautionary example of how a low-sample ablation can manufacture a directional artifact.
...
2. **The null holds under intervention, not just correlation.** A targeted ablation first hinted at a directional effect on hard windows, but at matched Monte-Carlo fidelity (100 samples, report §4.13) the hint reverses sign and disappears — it was 50-sample noise. The top-5 features add no measurable causal contribution to forecast quality, so the null is causal as well as correlational.
...
- **Causal ablation sample.** n = 56 hard windows is underpowered (49 % power, report §4.8). The matched-100-sample re-run (§4.13) resolves the *direction* — the pilot's positive hint reverses to a small negative — but nailing that negative estimate's sign would need multiple seeds.
- **`num_samples` Monte-Carlo noise (now controlled).** The original ablation used 50 samples for recon/ablation vs 100 for the natural baseline; the matched-100-sample re-run (§4.13) removes the asymmetry — and in doing so revealed that the 50-sample hard-window hint was itself MC noise.
- **Conformal baseline (now run).** The standard UQ comparison is done (report §4.9): the input-statistics probe *beats* the model's own conformalized predictive-interval width on selective prediction (30 % vs 9–11 % of oracle headroom), and conformal intervals under-cover on hard windows.
...
1. **Full-fidelity / larger backbone.** `chronos-t5-base` (200 M) is done at lean fidelity and the null persists (report §4.12); `chronos-t5-large` (710 M) and full-fidelity reruns need a GPU.
2. **Broader datasets.** The ET-T family (ETTh2/ETTm1/ETTm2) is replicated at lean fidelity with the null holding (report §4.11); Weather/Electricity/Traffic and full-fidelity confirmation remain. (The causal-ablation direction is already settled as null in §4.13, so this is about the *predictive* null's breadth, not chasing significance for a refuted effect.)
3. **Sharpen the suggestive lean cells.** A GPU rerun would resolve the two non-significant lean positives — the MULL channel (§4.14) and horizon H = 720 (§4.15) — which §4.13's lesson says to distrust until confirmed at full fidelity.
